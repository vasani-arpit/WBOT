  ...
  async function downloadAndStartThings() {
    ...
    const client = new Client({
      puppeteer: {
        executablePath: revisionInfo.executablePath,
        defaultViewport: null,
        headless: configs.appconfig.headless,
        devtools: false,
        args: [...pptrArgv],
        ...extraArguments,
      },
    ...
    client.on("ready", async () => {
      ...
      let server = configs.appconfig.server;
      if (server) {
    ...
    client.on("message", async (msg) => {
      ...
      if (msg.hasMedia && configs.appconfig.downloadMedia) {
        ...
        console.log(
          "Message doesn't have media or it is not enabled in bot.config.json"
        );
      }
...
async function sendReply({ msg, client, data, noMatch }) {
  let globalWebhook = configs.appconfig.webhook;

  if (noMatch) {
    if (configs.noMatch.length != 0) {
      let response = await getResponse(msg, configs.noMatch);
    ...
    if (captionStatus == true) {
      utils
        ...
        .then(async ({ fileMime, base64 }) => {
          ...
          var media = await new MessageMedia(fileMime, base64, file);
          if (!configs.appconfig.quoteMessageInReply) {
    ...
    response.forEach(async (itemResponse) => {
      ...
      if (itemResponse.files && itemResponse.files.length > 0) {
        itemResponse.files.forEach(async (itemFile) => {
          ...
          const base64Data = mimeTypeMatch
            ? itemFile.file.split(",")[1]
            : itemFile.file;

          const mimeType = mimeTypeMatch
            ? itemFile.file.split(":")[1].split(";")[0]
            : "image/jpg";
...
async function smartReply({ msg, client }) {
  // console.log(msg.body)
  const data = msg?.body;
  const list = configs.bot;

  //Don't reply is sender is blocked
  const senderNumber = msg.from.split("@")[0];
  var blockedNumbers = configs.blocked;
  var allowedNumbers = configs.allowed;
  ...
  if (
    Array.isArray(allowedNumbers) &&
    allowedNumbers.length > 0 &&
    !allowedNumbers.includes(senderNumber)
  ) {
    console.log(
      "Message received but user is not in allowed list so will not reply."
    );
    return;
  }

  // Don't do group reply if isGroupReply is off
  if (msg.id.participant && configs.appconfig.isGroupReply == false) {
    console.log(
